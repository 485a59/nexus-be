...
public class UFOPUtils {
    ...
    public static File getTempFile(String fileUrl) {
        ...
        return getFile(tempPath);
    }

    @NotNull
    private static File getFile(String tempPath) {
        File tempFile = new File(tempPath);
        File parentFile = tempFile.getParentFile();
        if (!parentFile.exists()) {
            boolean success = parentFile.mkdirs();
            if (!success) {
                log.info("创建文件夹 {} 失败", parentFile.getName());
            }
        }
    ...
    public static String getUploadFileUrl(String identifier, String extendName) {
        ...
        if (!dir.exists()) {
            ...
            if (!success) {
                log.error("创建文件夹 {} 失败", dir.getAbsolutePath());
            }
        }
